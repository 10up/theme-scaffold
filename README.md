...
5. [Browsersync](https://browsersync.io/) - Browsersync is used for effortless previews of your site across your local devices.
...
To allow Browsersync to load your local project, you'll need to add your local URL to the `config.json` file.
...
- open `config.json` and change the local URL.
- run `npm run start`
...
## Config
 ...
 To configure certain aspects of your developer experience, we have a configuration file in `config.json`. The current options you can set in there are:
 
 - `localUrl` - This should be set to the URL which will load your website. It corresponds to the `proxy` option in Browsersync: https://browsersync.io/docs/options#option-proxy 
 
 Additional settings can be placed in there. They will then be available in `gulpfile.babel.js` under the `config` object.
 
 ### Local config override
 You may want to override some settings that have been set in the `config.json` file. You can do so by placing you override settings in `config-local.json`. 
